...
Как `CNN`, так и трансформеры извлекают информацию о признаках путем пропускания исходного изображения через последовательность блоков. 
Блоки `CNN` состоят из последовательности сверток разного размера - более широкие свертки помогают извлекать информацию о локальных особенностях, а свертки 1 * 1 служат для изменения размерности тензора. Обычно `CNN` быстрее трансформеров за счет использования сокращения размерности, однако из-за ограниченного размера поля свертки, `CNN` слабо извлекают информацию о зависимости признаков на расстоянии. Трансформеры, напротив, имеют способность к генерализации: в отличие от `CNN`, сети `Transformer` сначала разделяют изображение на последовательность патчей (токенов), затем суммируют эти токены с позиционным кодированием для представления пространственной информации. 

В отличие от методов, используемых в классическом блоке `Transformer` для работы с необработанными изображениями с фиксированным разрешением, архитектура `ResT` имеет несколько преимуществ:
- Более эффективное использование памяти за счет понижения размерности входа с помощью сверточных блоков;
- Возможность обрабатывать изображения любого размера без использования интерполяции;

## CycleMLP

Модели на основе `MLP` отличаются от моделей на основе `CNN` и `Transformer`, поскольку они не прибегают ни к свертке, ни к слоям _self-attention_. Вместо этого они используют слои _MLP_ поверх участков пространственных измерений для агрегирования пространственного контекста. Модели на основе `MLP` имеют схожие структуры, но отличаются друг от друга деталями внутреннего устройства. Такие модели обеспечивают более эффективные вычисления, чем модели на основе трансформеров, поскольку им не нужно вычислять матрицы key, query, value. 
## Conformer
